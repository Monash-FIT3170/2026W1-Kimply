...
- A 30-second turn timer, live elimination feed, and level-up toasts
- Optional player accounts (register and sign in) and a global top-50 `/leaderboard`
...
├── server/
│   └── main.js               # Meteor startup, health, publications, indexes
├── imports/
│   ├── api/
...
│   │   ├── leaderboard.js         # LeaderboardCollection (per-round, definition only)
│   │   ├── gameEvents.js          # GameEventsCollection (level-up events)
│   │   ├── globalLeaderboard.js   # GlobalLeaderboardCollection + myStanding
...
│   └── ui/
│       ├── pages/            # Splash, PlayRoute, JoinRoom, PlayerLobby, GamePage, Account, GlobalLeaderboard
│       ├── components/       # design.jsx, ConfirmationPopup.jsx, ReconnectPopup.jsx
│       ├── Header.jsx
│       ├── Leaderboard.jsx        # live in-game leaderboard (players + rounds)
│       ├── EndLeaderboard.jsx     # end-of-game ranking
│       ├── EliminationFeed.jsx    # toast feed of eliminated players
...
| `/leaderboard` | `GlobalLeaderboard` | top-50 registered accounts |
| `*` | `Navigate to="/"` | catch-all |

`main.jsx` still declares **two identical `path="*"` routes** (one before `/account`, one after `/leaderboard`).
React Router v7 ranks by specificity rather than declaration order, so `/account` and `/leaderboard` still match, but the first catch-all is dead code.
...
| `GameEventsCollection` | `gameEvents` | `imports/api/gameEvents.js:3` |
| `GlobalLeaderboardCollection` | `globalLeaderboard` | `imports/api/globalLeaderboard.js:14` |
...
```js
{ gameId, roundId, name, accountId, lives: 3, attemptedSequence: [], currentStreak: 0,
...
**`leaderboard`** (written by `gameMethods.js` on a correct submit, append-only)
```js
{ gameId, playerId, name, lives, roundId, completedAt }
```

**`gameEvents`** (written by `rounds.advance` on level-up)
```js
{ gameId, type: 'level-up', playerId, playerName, level, createdAt }
```

**`globalLeaderboard`** (written by `recordGlobalResult` in `gameMethods.js`, capped at 50)
```js
{ accountId, displayName, bestRound, wins, gamesPlayed, achievedAt, updatedAt }
```
...
```
`gamesPlayed`, `wins`, and `bestRound` start at 0 and are updated by `recordGlobalResult` when a registered player's game ends. Guests have no `accountId` and never appear on the global board.
...
`globalLeaderboard` indexes (`accountId` unique; `{ bestRound: -1, wins: -1, achievedAt: 1 }`) are created in `imports/api/globalLeaderboard.js`, not `server/indexes.js`.
...
Game-scoped publications take `gameId` (the 5-character room PIN). `globalLeaderboard` is the exception: it publishes the capped top-50 snapshot with no `gameId`.
...
- Excluding `attemptedSequence` on `players` and `eliminations` matters because that field holds the answer a player submitted. Publishing it handed the correct sequence to everyone who had not played the round yet.
- Scoping by `gameId` is what keeps DDP fan-out proportional to room size rather than to the whole deployment.

Invalid `gameId` resolves to `this.ready()` rather than throwing, matching the `rooms.lobby` convention.
...
Files: `app/server/main.js`, `app/server/publications.js`, `app/imports/ui/pages/GamePage.jsx`, `app/imports/ui/pages/PlayerLobby.jsx`, `CLAUDE.md`.

`GamePage` keeps this branch's turn timer, level-up toasts (pinned **left** so they do not sit under `dev`'s fixed-right leaderboard toggle), `accountId` join, and `EliminationFeed`, plus `dev`'s collapsible live leaderboard and the no-`'demo'` `gameId` guard. Signed-in lobby navigation still carries `playerAccount`.
